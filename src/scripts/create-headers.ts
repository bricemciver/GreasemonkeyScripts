...
async function generateUserScriptHeaders(
  globPattern: string,
  outputDir: string,
): Promise<void> {
  ...
  for (const filePath of filePaths) {
    ...
    if (!parentDir) {
      console.error(
        `Error processing ${filePath.fullpath()}: parentDir not found`,
      );
      continue;
    }
    try {
      const metaData = JSON.parse(
        fs.readFileSync(filePath.fullpath(), "utf-8"),
      ) as Metadata;
...
if (!sourcePattern || !outputDir) {
  console.error(
    "Usage: npx ts-node create-headers.ts <source_pattern> <output_dir>",
  );
  process.exit(1);
}
