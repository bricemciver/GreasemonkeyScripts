...
const getASIN = (): string | null => {
  ...
  const dataAsinElement = document.querySelector<HTMLInputElement | HTMLDivElement>(
  ...
  const ldJsonScripts = document.querySelectorAll<HTMLScriptElement>('script[type="application/ld+json"]')
 ...
 */
const isProductPage = () => {
  const pathMatch = /\/(dp|gp\/product|gp\/aw\/d)\//.test(globalThis.location.pathname)
...
const addCamelChart = (camelCountry: string, asin: string, refresh: boolean, chartsContent: HTMLDivElement) => {
...
const createContainer = (camelCountry: string, asin: string, refresh: boolean, keepaDomain: number, tld: string) => {
