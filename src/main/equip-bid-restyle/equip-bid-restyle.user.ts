namespace EquipBidRestyle {
	...
	const createClose = () => {
		...
		close.style.transition =
			"background-color .02s ease-in-out, border-color .02s ease-in-out, color .02s ease-in-out";
		close.onclick = () => {
	...
	const createModal = (
		id: string,
		title: string,
		entries: { label: string; keyName: string }[],
	) => {
	...
	const scrollPictures = (direction: "right" | "left") => {
		...
		let focusedEntry = document.querySelector<HTMLLIElement>(
			".lot-list .list-group li.list-group-item.focused",
		);
		if (!focusedEntry) {
			// if nothing is focused yet, select the first item
			focusedEntry = document.querySelector<HTMLLIElement>(
	...
	const addToWatchlist = () => {
		// find the focused entry
		const focusedEntry = document.querySelector<HTMLLIElement>(
			".lot-list .list-group li.list-group-item.focused",
		);
		if (!focusedEntry) {
		...
		const watchlistButton = focusedEntry.querySelector<HTMLAnchorElement>(
	...
	const selectItem = (direction: "next" | "previous") => {
		...
		const selectedItem =
			direction === "next"
				...
				: focusedEntry.previousElementSibling;
		if (!selectedItem) {
	...
	export const collectItems = () => {
		...
		lotList
			.querySelector("hr")
			?.insertAdjacentElement("afterend", unorderedList);
	};

	const getDetails = async (href: string) => {
		let bidCount = "";
		let gallery: { image: string }[] = [];
		return new Promise<{ bidCount: string; gallery: { image: string }[] }>(
			(resolve, reject) => {
				GM_xmlhttpRequest({
					method: "GET",
					url: href,
	...
	export const additionalDetails = async () => {
		...
		const promises = Array.from(allItems).map(async (listItem) => {
			...
			const highBidder = Array.from(listItem.querySelectorAll("small")).filter(
				(item) => item.textContent?.includes("High Bidder"),
			)[0];
  ...
  export const defaultSort = () => {
	...
	export const addListeners = () => {
		const allListeners = (event: KeyboardEvent) => {
			...
			if (event.key === "n") {
				const nextButton =
					document.querySelector<HTMLAnchorElement>("li.next > a");
				nextButton?.click();
			}
			if (event.key === "p") {
				const prevButton =
					document.querySelector<HTMLAnchorElement>("li.previous > a");
				prevButton?.click();
			}
			if (event.key === "w") {
				addToWatchlist();
