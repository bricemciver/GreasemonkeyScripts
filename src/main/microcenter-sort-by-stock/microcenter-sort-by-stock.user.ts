namespace MicrocenterSortByStock {
	export const addOptionToMenu = () => {
		...
		const menu = document.querySelector<HTMLUListElement>(
			"div.searchActions > div.sorting ul.dropdown-menu",
		);
		if (menu) {
	...
	export const sortByStock = () => {
		...
		const selectedItem =
			document.querySelector<HTMLSpanElement>("span.sortByText");
		if (selectedItem) {
			selectedItem.textContent = "Stock";
		}
		// Get all of the entries on the page
		const entries =
			document.querySelectorAll<HTMLLIElement>("li.product_wrapper");
		// Sort the entries
	...
	const stockSortFunc = (entry1: HTMLLIElement, entry2: HTMLLIElement) => {
		...
		let entry1StockLi =
			entry1.querySelector<HTMLSpanElement>("span.inventoryCnt")?.textContent;
		if (entry1StockLi) {
			entry1StockLi = entry1StockLi.replace(" IN STOCK", "");
			entry1Stock = entry1StockLi.includes("25+")
				? 26
				: Number.parseInt(entry1StockLi);
		}
		// Get the stock from each entry
		let entry2StockLi =
			entry2.querySelector<HTMLSpanElement>("span.inventoryCnt")?.textContent;
		if (entry2StockLi) {
			entry2StockLi = entry2StockLi.replace(" IN STOCK", "");
			entry2Stock = entry2StockLi.includes("25+")
				? 26
				: Number.parseInt(entry2StockLi);
		}
