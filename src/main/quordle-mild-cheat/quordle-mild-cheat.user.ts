namespace QuordleMildCheat {
	 ...
	 */
	const cellRegEx =
		/'(\w+)' \(letter (\d+)\) is (in a different spot|correct|incorrect)/;
	...
	const extractGameBoard = (boardNum: number) => {
		...
		const board = document.querySelector<HTMLDivElement>(
			`div[role="table"][aria-label="Game Board ${boardNum}"]`,
		);
		// get rows
		if (board) {
		...
		for (const item of boardState) {
			if (item.status === "correct") {
				...
				tempWordList = tempWordList.filter(
					(word) =>
						word.charAt(item.position - 1).toUpperCase() ===
				...
				tempWordList = tempWordList.filter(
					(word) =>
						word.indexOf(item.letter.toUpperCase()) ===
						word.lastIndexOf(item.letter.toUpperCase()),
				);
			}
		}
