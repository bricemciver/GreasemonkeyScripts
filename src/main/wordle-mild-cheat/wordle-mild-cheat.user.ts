namespace WordleMildCheat {
			...
			if (
				...
				if (
					element.src.startsWith(
						"https://www.nytimes.com/games-assets/v2/wordle.",
					)
				) {
					// Get the script
					GM.xmlHttpRequest({
						...
						onload(response) {
							...
							const wordListStr = response.responseText.substring(
								begArray,
								endArray + 1,
							);
	...
	const extractGameBoard = () => {
		...
		const board = document.querySelector<HTMLDivElement>(
			"div[class^='Board-module_board__']",
		);
		if (board) {
			const tiles = board.querySelectorAll<HTMLDivElement>(
		...
		for (const item of boardState) {
			if (item.status === State.correct) {
				...
				tempWordList = tempWordList.filter(
					(word) =>
						word.charAt(item.position - 1).toLowerCase() ===
				...
				!boardState.some(
					({ letter, status }) =>
						(status === State.correct || status === State.diff) &&
						letter.toLowerCase() === item.letter.toLowerCase(),
				)
			) {
				// need to be careful here, only remove 'none' if it wasn't previously 'correct' or 'diff' (since it could be a second occurance)
				tempWordList = tempWordList.filter(
					(word) => word.indexOf(item.letter.toLowerCase()) === -1,
				);
			}
		}
