namespace AncestryRemovePaidHints {
	...
	const handleOfferPage = (db: IDBDatabase, link: Location) => {
		...
		if (dbidMatch) {
			...
			const getRequest = db
				.transaction("collections_os", "readonly")
				.objectStore("collections_os")
				.get(dbid);
			getRequest.onsuccess = () => {
				const result = getRequest.result;
				const putOS = db
					.transaction("collections_os", "readwrite")
					.objectStore("collections_os");
				if (result) {
					putOS.put({
						dbid,
					...
					getRequest.onsuccess = () => {
						...
						if (!result) {
							GM.xmlHttpRequest({
								...
								onreadystatechange(response) {
									if (
										response.readyState ===
										Tampermonkey.ReadyState.HeadersReceived
									) {
										// HeadersReceived
						...
						if (hide) {
							...
							if (li) {
								...
								if (
									section &&
									section.querySelectorAll("li[role='group']").length === 1
								) {
									section.remove();
								}
							}
	...
	const scanHints = (db: IDBDatabase, element: Element): void => {
		// get links
		const sseLinks =
			element.querySelectorAll<HTMLAnchorElement>("a[href*='sse.dll']");
		for (const link of sseLinks) {
			evalLink(db, link);
		}

		// remove family tree
		const familyTreeLinks = element.querySelectorAll<HTMLAnchorElement>(
			"a[href*='/family-tree/tree/']",
		);
			...
			for (const mutation of mutationList) {
				if (mutation.type === "childList") {
					for (const addedNode of mutation.addedNodes) {
						...
						if (
							element.innerHTML &&
							(element.innerHTML.indexOf("sse.dll") !== -1 ||
								element.innerHTML.indexOf("/family-tree/tree/") !== -1)
						) {
