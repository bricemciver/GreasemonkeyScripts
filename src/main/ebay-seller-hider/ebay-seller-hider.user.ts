namespace EbaySellerHider {
		...
		if (itemRegExp) {
			...
			if (parent) {
				...
				if (hideSponsored && parent.style.display === "list-item") {
					...
					const sponsoredSpan = Array.from(
						parent.querySelectorAll("span"),
					).find((item) => item.textContent === "Sponsored");
					if (sponsoredSpan) {
						const labelAttr =
							sponsoredSpan.parentElement?.parentElement?.getAttribute(
								"aria-labelledBy",
							);
	...
	export const getPresets = (): void => {
		...
		feedbackMin = Number.parseFloat(
			localStorage.getItem("feedbackMin") ?? "95.0",
		);
