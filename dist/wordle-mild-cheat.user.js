...
var processCell = (element) => {
	...
	var processGameBoard = (boardState) => {
		...
		for (const item of boardState) if (item.status === State.correct) tempWordList = tempWordList.filter((word) => word.charAt(item.position - 1).toLowerCase() === item.letter.toLowerCase());
		else if (item.status === State.diff) tempWordList = tempWordList.filter((word) => word.charAt(item.position - 1).toLowerCase() !== item.letter.toLowerCase() && word.indexOf(item.letter.toLowerCase()) !== -1);
		else if (item.status === State.none && !boardState.some(({ letter, status }) => (status === State.correct || status === State.diff) && letter.toLowerCase() === item.letter.toLowerCase())) tempWordList = tempWordList.filter((word) => word.indexOf(item.letter.toLowerCase()) === -1);
		return tempWordList;
	};
	var findAllowedWords = () => {
		fullWordList.push(...getItem("wordList", []));
		if (fullWordList.length === 0) new MutationObserver(callback).observe(document, {
			subtree: true,
			childList: true
		});
	};
