...
var getItem = (key, defaultVal) => {
	...
	var findAllowedWords = () => {
		...
		if (!wordBankWords.length || !allowedWords.length) {
			...
			if (script) GM.xmlHttpRequest({
				method: "GET",
				url: script.src,
				onload(response) {
					const text = response.responseText;
					const wordBankMatches = RegExp(wordBankRegEx).exec(text);
					if (wordBankMatches && wordBankMatches.length > 1) wordBankWords.push(...wordBankMatches[1].split(" "));
					const allowedMatches = RegExp(allowedRegEx).exec(text);
					if (allowedMatches && allowedMatches.length > 1) allowedWords.push(...allowedMatches[1].split(" "));
	...
	var createBoardList = (wordList, board, title) => {
		...
		for (const word of board) {
			...
			if (wordBankWords.some((item) => item === word)) listItem.classList.add("font-bold");
	...
	var addListeners = () => {
		document.addEventListener("keydown", (event) => {
			...
			if (event.key === "?") {
				...
				const boardArray = [];
				for (let i = 1; i < 5; i++) boardArray.push(processGameBoard(extractGameBoard(i)));
...
var cellRegEx = /'(\w+)' \(letter (\d+)\) is (in a different spot|correct|incorrect)/;
	...
	var processGameBoard = (boardState) => {
		...
		for (const item of boardState) if (item.status === "correct") tempWordList = tempWordList.filter((word) => word.charAt(item.position - 1).toUpperCase() === item.letter.toUpperCase());
		else if (item.status === "diff") tempWordList = tempWordList.filter((word) => word.charAt(item.position - 1).toUpperCase() !== item.letter.toUpperCase() && word.indexOf(item.letter.toUpperCase()) !== -1);
		else if (item.status === "none" && !boardState.some(({ letter, status }) => (status === "correct" || status === "diff") && letter === item.letter)) tempWordList = tempWordList.filter((word) => word.indexOf(item.letter.toUpperCase()) === -1);
		else if (item.status === "none" && boardState.some(({ letter, status }) => (status === "correct" || status === "diff") && letter === item.letter)) tempWordList = tempWordList.filter((word) => word.indexOf(item.letter.toUpperCase()) === word.lastIndexOf(item.letter.toUpperCase()));
		return tempWordList;
	};
	findAllowedWords();
	addListeners();
