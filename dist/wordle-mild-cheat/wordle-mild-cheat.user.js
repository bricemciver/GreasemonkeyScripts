  ...
  ;(WordleMildCheat2 => {
    ...
    const showWordlist = curWords => {
      ...
      if (!wordList) {
        ...
        ;(_a = style.sheet) == null
          ? void 0
          : _a.insertRule(`.dialog li {
        display: block;
        padding: 2px 0px;
      }`)
        ;(_b = style.sheet) == null
          ? void 0
          : _b.insertRule(`.dialog ul {
        list-style: none;
        margin: 4px 0px;
        position: relative;
        padding: 0px;
    ...
    const processGameBoard = boardState => {
      ...
      for (const item of boardState) {
        if (item.status === 0 /* correct */) {
          ...
          !boardState.some(
            ({ letter, status }) =>
              (status === 0 /* correct */ || status === 1) /* diff */ && letter.toLowerCase() === item.letter.toLowerCase(),
          )
        ) {
