...
var getASIN = () => {
		...
		const dataAsinElement = document.querySelector("input[data-asin]:not([data-asin=\"\"], div[data-asin]:not([data-asin=\"\"])");
		...
		for (const script of Array.from(ldJsonScripts)) try {
...
var isProductPage = () => {
		return /\/(dp|gp\/product|gp\/aw\/d)\//.test(globalThis.location.pathname) || document.getElementById("dp") !== null || document.getElementById("ppd") !== null;
	};
var addStyles = () => {
		const css = `
                #${SCRIPT_ID}-container {
...
var createChartElement = (title, linkUrl, imageUrl, altText, id, visible = true) => {
	...
	var addCamelChart = (camelCountry, asin, refresh, chartsContent) => {
		const camelChart = createChartElement("CamelCamelCamel", `https://${camelCountry}.camelcamelcamel.com/product/${asin}`, `https://charts.camelcamelcamel.com/${camelCountry}/${asin}/amazon-new-used.png?force=1&zero=0&w=600&h=400&desired=false&legend=1&ilt=1&tp=all&fo=0`, `CamelCamelCamel price history for ${asin}`, "camelChart", !refresh);
		if (refresh) {
	...
	var addKeepaChart = (keepaDomain, asin, tld, refresh, chartsContent) => {
		const keepaChart = createChartElement("Keepa", `https://keepa.com/#!product/${keepaDomain}-${asin}`, `https://graph.keepa.com/pricehistory.png?used=1&asin=${asin}&domain=${tld}`, `Keepa price history for ${asin}`, "keepaChart", !refresh);
		if (refresh) {
