...
;(() => {
  ...
  var init_index_modern = __esm({
    'node_modules/@badrap/result/dist/index.modern.mjs'() {
      r = class {
        unwrap(r2, t2) {
          const e2 = this._chain(
            t3 => n.ok(r2 ? r2(t3) : t3),
            r3 => (t2 ? n.ok(t2(r3)) : n.err(r3)),
          )
  ...
  var init_chess = __esm({
    'node_modules/chessops/dist/esm/chess.js'() {
      ...
      attacksTo = (square, attacker, board, occupied) =>
          ...
          if (this.castlingRights.has(square)) {
            ...
            for (const color of COLORS) {
              for (const side of CASTLING_SIDES) {
                if (this.rook[color][side] === square) this.rook[color][side] = void 0
              }
            }
          }
        }
      ...
      Position = class {
        ...
        hasInsufficientMaterial(color) {
          ...
          if (this.board[color].intersects(this.board.bishop)) {
            const sameColor =
              !this.board.bishop.intersects(SquareSet.darkSquares()) || !this.board.bishop.intersects(SquareSet.lightSquares())
  ...
  var init_fen = __esm({
    'node_modules/chessops/dist/esm/fen.js'() {
      ...
      parseBoardFen = boardPart => {
        ...
        for (let i = 0; i < boardPart.length; i++) {
          ...
          if (c === '/' && file === 8) {
            ...
            if (step > 0) file += step
              ...
              if (file >= 8 || rank < 0) return n.err(new FenError(InvalidFen.Board))
        ...
        if (rank !== 0 || file !== 8) return n.err(new FenError(InvalidFen.Board))
        return n.ok(board)
      }
      parsePockets = pocketPart => {
        if (pocketPart.length > 64) return n.err(new FenError(InvalidFen.Pockets))
        const pockets = Material.empty()
        for (const c of pocketPart) {
          const piece = charToPiece(c)
          if (!piece) return n.err(new FenError(InvalidFen.Pockets))
      ...
      parseCastlingFen = (board, castlingPart) => {
        ...
        if (castlingPart === '-') return n.ok(castlingRights)
      ...
      parseRemainingChecks = part => {
        ...
        } else return n.err(new FenError(InvalidFen.RemainingChecks))
      }
      parseFen = fen => {
        ...
        return board.chain(board2 => {
          ...
          if (!defined(halfmoves)) return n.err(new FenError(InvalidFen.Halfmoves))
          const fullmovesPart = parts.shift()
          const fullmoves = defined(fullmovesPart) ? parseSmallUint(fullmovesPart) : 1
          if (!defined(fullmoves)) return n.err(new FenError(InvalidFen.Fullmoves))
          const remainingChecksPart = parts.shift()
          let remainingChecks = n.ok(void 0)
          if (defined(remainingChecksPart)) {
            if (defined(earlyRemainingChecks)) return n.err(new FenError(InvalidFen.RemainingChecks))
            remainingChecks = parseRemainingChecks(remainingChecksPart)
          } else if (defined(earlyRemainingChecks)) {
            remainingChecks = earlyRemainingChecks
          }
          if (parts.length > 0) return n.err(new FenError(InvalidFen.Fen))
          return pockets.chain(pockets2 =>
            castlingRights.chain(castlingRights2 =>
              remainingChecks.map(remainingChecks2 => {
                return {
                  board: board2,
                  pockets: pockets2,
                  turn,
      ...
      makePiece = piece => {
        ...
        if (piece.color === 'white') r2 = r2.toUpperCase()
        if (piece.promoted) r2 += '~'
        return r2
      }
      makeBoardFen = board => {
        let fen = ''
  ...
  var init_variant = __esm({
    'node_modules/chessops/dist/esm/variant.js'() {
      ...
      Crazyhouse = class extends Position {
        ...
        dropDests(ctx) {
          ...
          const mask = this.board.occupied
            .complement()
            .intersect(
              ((_a = this.pockets) === null || _a === void 0 ? void 0 : _a[this.turn].hasNonPawns())
                ? SquareSet.full()
      ...
      Atomic = class extends Position {
        ...
        dests(square, ctx) {
          ...
          for (const to of pseudoDests(this, square, ctx)) {
            ...
            if (
              defined(ourKing) &&
              (!defined(after.board.kingOf(after.turn)) || after.kingAttackers(ourKing, after.turn, after.board.occupied).isEmpty())
            ) {
              dests = dests.with(to)
            }
          }
      ...
      Antichess = class extends Position {
        ...
        hasInsufficientMaterial(color) {
          ...
          if (this.board.occupied.equals(this.board.knight) && this.board.occupied.size() === 2) {
            return (
              (this.board.white.intersects(SquareSet.lightSquares()) !== this.board.black.intersects(SquareSet.darkSquares())) !==
              (this.turn === color)
            )
          }
          return false
      ...
      RacingKings = class extends Position {
        ...
        validate() {
          if (this.isCheck() || this.board.pawn.nonEmpty()) return n.err(new PositionError(IllegalSetup.Variant))
      ...
      Horde = class extends Position {
        ...
        validate() {
          if (this.board.occupied.isEmpty()) return n.err(new PositionError(IllegalSetup.Empty))
          if (this.board.king.size() !== 1) return n.err(new PositionError(IllegalSetup.Kings))
          const otherKing = this.board.kingOf(opposite(this.turn))
        ...
        hasInsufficientMaterial(color) {
          ...
          const hordeNum =
            horde.pawn + horde.knight + horde.rook + horde.queen + Math.min(hordeBishops('dark'), 2) + Math.min(hordeBishops('light'), 2)
          const pieces = MaterialSide.fromBoard(this.board, opposite(color))
          const piecesBishops = squareColor =>
  ...
  var init_pgn = __esm({
    'node_modules/chessops/dist/esm/pgn.js'() {
      ...
      startingPosition = headers => {
        ...
        if (!rules) return n.err(new PositionError(IllegalSetup.Variant))
        const fen = headers.get('FEN')
        if (fen) return parseFen(fen).chain(setup => setupPosition(rules, setup))
        else return n.ok(defaultPosition(rules))
      }
  ...
  var require_lichess_show_common_move_user = __commonJS({
    'src/main/lichess-show-common-move/lichess-show-common-move.user.ts'(exports) {
      ...
      ;(LichessShowCommonMove2 => {
        ...
        const getMoveList = (node, observer) =>
