...
var indexAction = (action) => {
		if (index === 0 && prevLink && action === "minus") prevLink.click();
		else if (index > lots.length - 2 && nextLink && action === "plus") nextLink.click();
		else if (action === "plus") {
			lots[index].classList.toggle("selected_lot", false);
			index++;
			lots[index].classList.toggle("selected_lot", true);
			lots[index].scrollIntoView({ block: "center" });
	...
	var addToWatchList = () => {
		...
		if (watchlistButton) watchlistButton.click();
	};
	var openInNewTab = () => {
    ...
    z-index: 2
      ...
      outline-offset: -2px
		...
		if (lotList) makeLotItemsIntoCards(lotList);
	};
	var makeLotItemsIntoCards = (lotList) => {
		let newNode = null;
	...
	var helpTopics = [
		...
		},
		{
			...
			items: [{
				key: "j",
				description: "Scroll to next auction item"
			}, {
				key: "k",
				description: "Scroll to previous auction item"
			}]
		},
		{
			...
			items: [{
				...
				key: "v",
				description: "Open item in a new tab"
			}]
		}
	];
	var index = 0;
	window.addEventListener("keydown", (event) => {
		if (event.code === "KeyJ") indexAction("plus");
		if (event.code === "KeyK") indexAction("minus");
		if (event.code === "KeyW") addToWatchList();
		if (event.code === "KeyV") openInNewTab();
		if (event.code === "Slash" && event.shiftKey) showHelp();
