...
var byteToHex = [];
	...
	var CSRFPattern = /name=["']csrfToken["'][^>]*value=["']([^"']+)["']/i;
	var ContentLengthPattern = /content-length:\s*(\d+)/i;
	var EPUB_CONTENT_TYPE = "application/epub+zip";
	var TOAST_TIMEOUT_MS = 5e3;
	var AMAZON_SENDTOKINDLE_URL = "https://www.amazon.com/sendtokindle";
	var LevelToClassMap = {
	...
	var HTTP_OK = 200;
	var ensureStatus = (response, expectedStatus = HTTP_OK) => {
		if (response.status !== expectedStatus) throw new Error(`Request returned status ${response.status}`);
	};
	var parseJsonResponse = (responseText) => {
		try {
	...
	var getCsrfToken = async () => {
		...
		const html = (await GM.xmlHttpRequest({
			method: "GET",
			url: AMAZON_SENDTOKINDLE_URL
		})).responseText || "";
		if (/<span[^>]*id=["']s2k-dnd-sign-in-button-text["'][^>]*>[\s\S]*?Sign\s*in[\s\S]*?<\/span>/i.test(html)) {
		...
		log("Warning: Could not find CSRF token in Amazon page excerpt:", (html.substring(0, 1e3) || "").split(/\s+/).join(" "));
		throw new Error("CSRF_NOT_FOUND");
	};
	var log = (message, data) => {
		const timestamp = ( new Date()).toISOString();
		if (data) console.info(`[Gutenberg Send to Kindle ${timestamp}] ${message}`, data);
		else console.info(`[Gutenberg Send to Kindle ${timestamp}] ${message}`);
	};
	var injectStyles = () => {
	...
	var getEpubInfo = () => {
		...
		for (const a of epub3Candidates) if ((a.textContent || "").trim() === exactEpub3Text) {
			epubLink = a;
		...
		try {
			...
			log("Title/author extracted from document.title", {
				title,
				author
			});
		} catch (e) {
	...
	var headEpub = async (url) => {
		log(`Performing HEAD request for EPUB: ${url}`);
		const headers = (await GM.xmlHttpRequest({
			method: "HEAD",
			url
		}).catch((e) => {
	...
	var initSendToKindle = async (fileSize, csrfToken) => {
		...
		if (data.statusCode !== 0) throw new Error(`Init failed with status code: ${data.statusCode}`);
		return data;
	};
	var uploadEpub = async (uploadUrl, epubData, csrfToken) => {
		...
		ensureStatus(response);
		const data = parseJsonResponse(new TextDecoder().decode(response.response));
		log("Upload response received", data);
		return data;
	};
	var sendToKindle = async (stkToken, title, author, contentLength, filename, csrfToken) => {
	...
	var sendEpubToKindle = async () => {
		try {
			...
			log("Send to Kindle completed successfully", await sendToKindle(initData.stkToken, epubInfo.title, epubInfo.author, uploadData["Content-Length"], epubInfo.filename, csrfToken));
	...
	var addSendToKindleButton = () => {
		let epubLink = null;
		const epubLinks = document.querySelectorAll("a[class*=\"link\"][title*=\"Download\"]");
		for (const link of Array.from(epubLinks)) if (link.textContent.includes("Send-to-Kindle")) {
			epubLink = link;
			break;
		}
		if (!epubLink) {
