...
var getItem = (key, defaultVal) => {
	...
	var callback = (mutationList, mutationObserver) => {
		for (const mutation of mutationList) if (mutation.addedNodes.length > 0 && mutation.addedNodes[0].nodeType === Node.ELEMENT_NODE && mutation.addedNodes[0].nodeName === "SCRIPT") {
			...
			if (element.type === "module") {
				GM.xmlHttpRequest({
					...
					onload(response) {
						...
						if (wordBankMatches && wordBankMatches.length > 1) wordBankWords.push(...wordBankMatches[1].split(" ").map((word) => word.toUpperCase()));
						const allowedMatches = RegExp(allowedRegEx).exec(text);
						if (allowedMatches && allowedMatches.length > 1) allowedWords.push(...allowedMatches[1].split(" ").map((word) => word.toUpperCase()));
	...
	var findAllowedWords = () => {
		...
		if (!wordBankWords.length || !allowedWords.length) new MutationObserver(callback).observe(document, {
			subtree: true,
			childList: true
		});
	};
	...
	var createBoardList = (wordList, board, title) => {
		...
		for (const word of board) {
			...
			if (wordBankWords.some((item) => item === word)) listItem.attributeStyleMap.set("font-weight", 700);
	...
	var showWordlist = (...boards) => {
		...
		boards.forEach((board, index) => {
			if (wordList) createBoardList(wordList, board, `Board ${index + 1}`);
		});
	...
	var processCell = (pos, element) => {
		...
		if (element.classList.contains("exact-match")) return {
			letter: letter.trim(),
			position: pos,
			status: State.correct
		};
		if (element.classList.contains("word-match")) return {
			letter: letter.trim(),
			position: pos,
			status: State.diff
		};
		if (letter.length > 0 && letter === letter.toUpperCase() && letter !== letter.toLowerCase()) return {
			letter: letter.trim(),
			position: pos,
			status: State.none
		};
	...
	var addListeners = () => {
		document.addEventListener("keydown", (event) => {
			...
			if (event.key === "?") {
				...
				const boardArray = [];
				for (let i = 1; i < 9; i++) boardArray.push(processGameBoard(extractGameBoard(i)));
	...
	var processGameBoard = (boardState) => {
		...
		for (const item of boardState) if (item.status === State.correct) tempWordList = tempWordList.filter((word) => word.charAt(item.position).toUpperCase() === item.letter.toUpperCase());
		else if (item.status === State.diff) tempWordList = tempWordList.filter((word) => word.charAt(item.position).toUpperCase() !== item.letter.toUpperCase() && word.indexOf(item.letter.toUpperCase()) !== -1);
		else if (item.status === State.none && !boardState.some(({ letter, status }) => (status === State.correct || status === State.diff) && letter === item.letter)) tempWordList = tempWordList.filter((word) => word.indexOf(item.letter.toUpperCase()) === -1);
		else if (item.status === State.none && boardState.some(({ letter, status }) => (status === State.correct || status === State.diff) && letter === item.letter)) tempWordList = tempWordList.filter((word) => word.indexOf(item.letter.toUpperCase()) === word.lastIndexOf(item.letter.toUpperCase()));
		return tempWordList;
	};
	findAllowedWords();
	addListeners();
