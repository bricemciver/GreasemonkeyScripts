      ...
      for (const item of boardState) {
        if (item.status === 0 /* correct */) {
          ...
          tempWordList = tempWordList.filter(
            word =>
              word.charAt(item.position).toUpperCase() !== item.letter.toUpperCase() && word.indexOf(item.letter.toUpperCase()) !== -1,
          )
        } else if (
          item.status === 2 /* none */ &&
          !boardState.some(({ letter, status }) => (status === 0 /* correct */ || status === 1) /* diff */ && letter === item.letter)
        ) {
