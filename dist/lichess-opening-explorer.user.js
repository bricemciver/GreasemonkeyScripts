...
;(function () {
  ...
  var attacksTo = (square, attacker, board, occupied) =>
  ...
  var Castles = class Castles {
    ...
    discardRook(square) {
      if (this.castlingRights.has(square)) {
        ...
        for (const color of COLORS)
          for (const side of CASTLING_SIDES) if (this.rook[color][side] === square) this.rook[color][side] = void 0
      }
    }
    discardColor(color) {
  ...
  var Crazyhouse = class extends Position {
    ...
    validate() {
      return super.validate().chain((_) => {
        ...
        if (
          (((_b = this.pockets) === null || _b === void 0 ? void 0 : _b.size()) || 0) + this.board.occupied.size() >
          64
        )
  ...
  var ThreeCheck = class extends Position {
    ...
    setupUnchecked(setup) {
      ...
      this.remainingChecks =
        ((_a = setup.remainingChecks) === null || _a === void 0 ? void 0 : _a.clone()) || RemainingChecks.default()
    }
    static default() {
      const pos = new this()
      pos.reset()
  ...
  var Horde = class extends Position {
    ...
    hasInsufficientMaterial(color) {
      ...
      const coloredSquares = (squareColor) =>
      ...
      const hordeBishops = (squareColor) =>
      ...
      if (hordeNum === 1) {
        ...
        if (piecesNum === 1) return true
          ...
          return !(pieces.pawn >= 1 || pieces.bishop >= 1 || (pieces.knight >= 1 && pieces.rook + pieces.queen >= 1))
  ...
  var main = async () => {
            ...
            if (node instanceof HTMLElement && node.matches('rm6, l4x, kwdb')) {
              ...
              if (result.length > 0) {
                const data = await fetchWithCache(
                  `https://explorer.lichess.ovh/masters?since=2008&play=${result.join(',')}`,
                )
                if (data?.ok) {
    ...
    if (initialResult.length > 0) {
      const data = await fetchWithCache(
        `https://explorer.lichess.ovh/masters?since=2008&play=${initialResult.join(',')}`,
      )
      if (data) {
        const chessDBResult = await data.json()
        if (chessDBResult.moves.length > 0) {
