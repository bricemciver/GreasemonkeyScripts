...
kingOf(color) {
	...
	var Crazyhouse = class extends Position {
		...
		validate() {
			return super.validate().chain((_) => {
				var _a, _b;
				if ((_a = this.pockets) === null || _a === void 0 ? void 0 : _a.count("king")) return Result.err(new PositionError(IllegalSetup.Kings));
				if ((((_b = this.pockets) === null || _b === void 0 ? void 0 : _b.size()) || 0) + this.board.occupied.size() > 64) return Result.err(new PositionError(IllegalSetup.Variant));
				return Result.ok(void 0);
			});
		}
		hasInsufficientMaterial(color) {
	...
	var Horde = class extends Position {
		...
		validate() {
			...
			const otherKing = this.board.kingOf(opposite(this.turn));
			if (defined(otherKing) && this.kingAttackers(otherKing, this.turn, this.board.occupied).nonEmpty()) return Result.err(new PositionError(IllegalSetup.OppositeCheck));
			for (const color of COLORS) {
				const backranks = this.board.pieces(color, "king").isEmpty() ? SquareSet.backrank(opposite(color)) : SquareSet.backranks();
				if (this.board.pieces(color, "pawn").intersects(backranks)) return Result.err(new PositionError(IllegalSetup.PawnsOnBackrank));
			}
			return Result.ok(void 0);
		}
		hasInsufficientMaterial(color) {
...
var defaultGame = (initHeaders = defaultHeaders) => ({
	...
	var getLichessGame = () => {
		...
		const pos = startingPosition(defaultGame().headers).unwrap();
		const uciMoves = [];
	...
	var main = async () => {
		...
		const callback = async (mutationsList, internalObserver) => {
			for (const mutation of mutationsList) if (mutation.type === "childList") {
				for (const node of mutation.addedNodes) if (node instanceof HTMLElement && node.matches("rm6, l4x, kwdb")) {
					console.info("Move list updated, extracting position...");
					const result = getLichessGame();
					if (result.length > 0) {
						const data = await fetchWithCache(`https://explorer.lichess.ovh/masters?since=2008&play=${result.join(",")}`);
						if (data?.ok) {
						...
						} else console.warn("Failed to fetch data from Lichess Opening Explorer.");
					}
				}
			}
		};
		...
		if (initialResult.length > 0) {
			const data = await fetchWithCache(`https://explorer.lichess.ovh/masters?since=2008&play=${initialResult.join(",")}`);
			if (data) {
				const chessDBResult = await data.json();
				if (chessDBResult.moves.length > 0) {
					updateMoveDisplay(chessDBResult.moves[0].uci);
